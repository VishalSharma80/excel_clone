...
//    cellInfo["text-decoration"] ? $(".icon-underline").addClass("selected") : $(".icon-underline").removeClass("selected")
   ...
   $(".font-family-selector").val(cellInfo["font-size"])
...
function getRowCol(ele){
    let idAary  = $(ele).attr("id").split("-")
    let rowId = parseInt(idAary[1]);
    let colId = parseInt(idAary[3]);
...
// if (cellData[selectedSheet][rowId] && Object.keys(cellData[selectedSheet][rowId]).length == 0) {
...
function selectsheet(ele) {
    ...
    //         // let colCode = $(`.colId-${j}`).attr("id").split("-")[1];
    //         let column = $(`<div class="r-input-cell" contenteditable="false" id = "row-${i}-col-${j}" data="code-${colCode}"></div>`);
    //         row.append(column);
    //     }
    //     $(".input-cell-container").append(row);
    // }
